...
<div id="wrapper">
	...
	<br><button type="submit" onclick="ServletInserisciDipendente" onsubmit="return validateForm()">Inserisci</button>
</form>
...
<form  action="GestisciDipendente" method="post">
	...
	<select name="DeleteDipendente">
	 <%
	 	for(int i=0;i<dipendenti.size();i++){%>
	 		<option value="<%=dipendenti.get(i).getId()%>"><%=dipendenti.get(i).getNome()+" "+dipendenti.get(i).getCognome()%></option>
	 		<%d=dipendenti.get(i);%>
	 	<% }%>
	</select> <button type="submit" onclick="GestisciDipendente">Elimina</button><br><br>
	</form>
	<form action="GestisciDipendente" method="post">
	...
	Password <input type="text" name="modificaPassword" value="<%=d.getPassword()%>"><%d.getPassword();%><br><br>
	 <button type="submit" onclick="GestisciDipendente">Modifica</button>
	</form>
        </div>
